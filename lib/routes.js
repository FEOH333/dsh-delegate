 ...
 * (`$DSH_HOME/subagent-model.json`, default `~/.dsh`), list the selectable models from the
...
export function registerRoutesOnce(ctx, deps) {
	...
	// Optional, not injected: a headless / TUI profile has no web server, and
	// the route family exists only for the browser card. Skipping it must never
	// keep the delegation tools from mounting.
