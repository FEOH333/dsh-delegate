...
/**
 * First text of an optional content-block array — or of an already-joined
 * string, which is what the official `settleRun` returns for a background run
 * (`runOutcome.output` is `finalText(...)`, not blocks). Accepting both shapes
 * is why a one-shot background run's roster summary is no longer always empty.
 * @param value - content blocks, a joined string, or anything else (→ "").
 * @param limit - optional truncation length.
 */
export function firstText(value, limit) {
