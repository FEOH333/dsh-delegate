...
/**
 * Settings namespace this plugin exposes so the Settings > Plugins page
 * (dsh ≥ 0.1.1-rc.2) dispatches our plugin-config card: the page renders the
 * intersection of "namespaces the Host serves" and "cards registered into
 * `settings.plugin.item`", keyed by the namespace name. The card's DATA still
 * flows through this plugin's own `/api/subagent-model` routes and
 * `~/.dsh/subagent-model.json` (unchanged); this namespace is a presence
 * declaration only — a served namespace with no card renders nothing, and a
 * card without a served namespace is never dispatched.
 */
...
/**
 * Register the settings namespace ONCE per process. Follows the official
 * `installSettingsSection` pattern (the same link the shell / web-search
 * plugin cards use); hooks are inert because the plugin owns its config via
 * `config-store`, and installation failure only hides the card, never breaks
 * the tools.
 */
