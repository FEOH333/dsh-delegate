		...
		disposeTool = ctx.tools.register(defineTool({
			...
			output: {
				...
				render: (_args, value) => [{
					...
					text: value.kind === "background"
						? `started background subagent task ${value.jobId} (task ${value.task_id ?? value.runId})\n[run: ${value.runId}]`
						: value.kind === "continuable"
							? `started subagent ${value.subagentId} (task ${value.task_id ?? value.runId})\n[run: ${value.runId}]`
			...
			async execute(args, exec) {
				...
				const model = (typeof args.model === "string" && args.model !== "")
					? args.model
					: (typeof user.defaultModel === "string" && user.defaultModel !== "")
						? user.defaultModel
						: parentModel;
				...
				if (background) {
					if (continuable) {
						...
						return { kind: "continuable", subagentId: started.childId, runId, task_id: taskId };
					}
				...
				try {
					...
					return { ...result, runId, task_id: taskId };
				} catch (error) {
					if (track) {
						try {
