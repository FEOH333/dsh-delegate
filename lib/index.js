...
import { installSettingsSectionOnce, resetSettingsSectionForTesting } from "./settings-ns.js";

/** Plugin identity registered by the Loader. */
export const name = "tool-subagent-model";
/**
 * Host-plane services this row needs; all ship with dsh-base.
 *
 * `webServer` is deliberately NOT injected: Cordis defers a row until every
 * injected service exists, so requiring it would keep the delegation tools
 * from ever mounting in a headless / TUI profile (no web server). The route
 * family is the browser card's transport only, so it is resolved optionally in
 * `registerRoutesOnce` and simply skipped when absent.
 */
...
/**
 * The parent's CURRENT route options. `parent.options` holds the creation-time
 * route, which goes stale after a mid-session model switch; the session's
 * latest request header owns the live route instead.
 *
 * dsh 0.1.2 exposes the official helper `parentAgentOptionsForDelegation` for
 * exactly this, but 0.1.1-rc.2 does NOT export it — importing it statically
 * would be a link-time `SyntaxError` on that version, the same failure class
 * this release fixes. So the seam is read directly, defensively: a session
 * without `requestHeader` (0.1.1-rc.2) or without a request yet degrades to the
 * creation options, which is the pre-v0.3.8 behavior.
 * @param parent - the calling agent.
 * @returns detached options carrying at least `provider` / `model`.
 */
function parentRouteOptions(parent) {
	try {
		const config = parent.session?.requestHeader?.()?.config;
		if (config !== null && typeof config === "object" && !Array.isArray(config)) {
			return {
				provider: typeof config.provider === "string" ? config.provider : parent.options?.provider,
				model: typeof config.model === "string" ? config.model : parent.options?.model
			};
		}
	} catch {
		// a session without a request header (or any other shape) degrades
	}
	return { ...parent.options };
}

/**
 * Read the LIVE model catalog from the `llm` service — the routes that
 * actually have a registered adapter right now.
 *
 * This is the deployment-agnostic source: the `llm-pi-ai` settings section is
 * only ONE adapter family. A deployment may serve its routes from
 * `llm-deepseek`, a custom adapter, or several families at once, and on those
 * the settings section above is absent (unregistered), which used to leave the
 * `model` enum empty and silently routed every explicit id to the parent's
 * route. `llm.listProviders()` + `llm.listModels(route)` report the routable
 * truth regardless of which plugin owns the settings namespace.
 *
 * Advisory by construction: an absent service, a throwing provider, or an
 * adapter that advertises nothing degrades to `[]` (the caller then falls back
 * to the settings-section catalog), never an error.
 *
 * @param ctx - plugin context; `llm` is read optionally.
 * @returns the flattened live catalog `{ provider, model, label }`, in route order.
 */
export async function readLiveEntries(ctx) {
	...
	for (const provider of providers) {
		const route = provider !== null && typeof provider === "object" && typeof provider.id === "string" ? provider.id : "";
		if (route === "") continue;
		let models;
		try {
			models = await llm.listModels(route);
		} catch {
			continue; // one unreadable route must not hide the others
		}
...
// ── settings namespace (the v0.3.5 card-visibility fix) ──────────────────────
// Implementation lives in lib/settings-ns.js; the identifiers stay reachable
// from this entry module for the loader graph and the smoke suite.
export { installSettingsSectionOnce, resetSettingsSectionForTesting } from "./settings-ns.js";
...
export function apply(ctx, config) {
	...
	let entries = [];
	/**
	 * Which seam produced `entries`: `"live"` (the `llm` service, authoritative
	 * for routable routes) or `"settings"` (`llm-pi-ai` section fallback). Once
	 * a live read succeeded, the synchronous settings path must not overwrite
	 * it with a list of configured-but-not-routable ids.
	 */
	let catalogSource = "settings";

	/**
	 * Read the `llm-pi-ai` settings section — the pre-0.1.2 catalog path, and
	 * the fallback when the `llm` service reports no routable route.
	 */
	const readEntries = () => {
	...
	// this row activates (entries mount concurrently), and the live `llm`
	// routes are read asynchronously right after — `refreshModelList` is
	// re-run on every execute and on settings changes, so the schema heals
	// itself even when this first read sees nothing.
	entries = readEntries();

	/** Adopt a freshly computed catalog, rebuilding the tool when it changed. */
	...
	/** Synchronous settings-section refresh (instant enum on boot). */
	const refreshModelList = () => {
		if (catalogSource === "live") return; // live routes are authoritative
		applyEntries(readEntries());
	};

	/** In-flight guard so concurrent triggers share one live read. */
	let catalogInFlight;
	/**
	 * Refresh the catalog from the live `llm` routes, falling back to the
	 * settings section when no route advertises models. Single-flight: the
	 * boot read, every settings change, and an empty-catalog execute share it.
	 */
	const refreshCatalog = () => {
		if (catalogInFlight !== undefined) return catalogInFlight;
		catalogInFlight = (async () => {
			let live = [];
			try {
				live = await readLiveEntries(ctx);
			} catch {
				live = [];
			}
			if (live.length > 0) {
			...
			// a catalog read must never surface as a delegation failure
		}).finally(() => {
		...
		disposeTool = ctx.tools.register(defineTool({
			...
			parameters: {
				...
				model: modelIds.length > 0 ? {
					...
					type: "string",
					description: "The child agent's model id. No live provider route advertises a model list right now (llm service / llm-pi-ai settings), so any id is accepted and routed to your current provider; omit to inherit your current model."
				},
			...
			async execute(args, exec) {
				...
				// a stale schema never blocks a valid call. An empty catalog
				// triggers one background live read (fire-and-forget, so a slow
				// adapter can never delay the delegation).
				refreshModelList();
				if (entries.length === 0) void refreshCatalog();
				// User defaults from Settings > Plugins ($DSH_HOME/subagent-model.json):
				// tool argument > user default > inherited/row value.
				const user = readUserConfig();
				const parentOptions = parentRouteOptions(parent);
	...
	// skipped so the request-cache prefix stays stable. Every `llm-*`
	// namespace can change which routes are live, so the live read is
	// re-run too (the settings-section path stays the synchronous fallback).
	ctx.on("settings/updated", (ns) => {
		if (typeof ns === "string" && ns.startsWith("llm")) {
			refreshModelList();
			void refreshCatalog();
		}
	});

	// The authoritative topology signal: an adapter registered or unregistered
	// routes (or the configurable-provider directory changed). Re-read the live
	// catalog so the `model` / `provider` enums track what is routable NOW.
	ctx.on("llm/adapters-updated", () => {
		void refreshCatalog();
	});

	// Boot catalog: the live `llm` routes land asynchronously, so the enum is
	// filled from them (falling back to the settings section) shortly after
	// this row activates. Fire-and-forget: the row must not wait on it.
	void refreshCatalog();
	...
	// Settings namespace (v0.3.5): makes the Settings > Plugins page dispatch
	// our plugin-config card (keyed slots need a served namespace). Also
	// single-flight across rows; absence only hides the card.
	installSettingsSectionOnce(ctx, config);
