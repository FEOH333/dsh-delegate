 ...
 *
 * Registers a dedicated Settings page (`settings.section` list slot, v0.3.6),
 * one card into the Settings > Plugins section via the keyed
 * `settings.plugin.item` slot (dsh ≥ 0.1.1-rc.2), and conversation-flow
 * tool-call cards via the open `tool.call.toolview` keyed slot: a run card
 * per delegation (status from the host roster, child-session navigation,
 * persona/dependency detail) and a roster card for `subagent_status`.
 *
 * The cards talk ONLY to the plugin's own /api/subagent-model routes (plain
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			lockDefaultModel: "锁定默认模型",
			lockDefaultModelHint: "开启后忽略每次调用指定的 model，一律使用上面的默认模型（未配置默认值时无效）",
			srcArg: "显式指定",
			srcDefault: "默认",
			srcInherited: "继承",
			save: "保存",
			...
			saved: "已保存",
			loading: "加载中...",
			note: "每次调用仍可传 model / max_tokens / provider 临时覆盖；此处是省略参数时的默认值。模型列表来自当前可路由的提供商路由（llm 服务）。",
			expand: "展开",
			collapse: "收起",
		...
		const en = {
			...
			lockDefaultModel: "Lock default model",
			lockDefaultModelHint: "When on, per-call `model` is ignored and the default above always wins (no-op without a default set)",
			srcArg: "explicit",
			srcDefault: "default",
			srcInherited: "inherited",
			save: "Save",
			saving: "Saving...",
			saved: "Saved",
			loading: "Loading...",
			note: "Per-call model / max_tokens / provider still override; these are defaults when omitted. The model list comes from the currently routable provider routes (the llm service).",
			expand: "Expand",
			collapse: "Collapse",
		...
		function SubagentModelCard(props) {
			...
			useEffect(() => {
				...
				(async () => {
					try {
						...
						setDraftLock(value.lockDefaultModel === true);
					} catch (error) {
			...
			return h("li", { style: cardStyles.card },
				...
				open ? h("div", { style: cardStyles.body },
					...
					h("div", { style: cardStyles.row },
						h("label", { style: cardStyles.label },
							h("input", { type: "checkbox", style: { marginRight: "6px" }, checked: draftLock, onChange: (event) => setDraftLock(event.target.checked) }),
							t("lockDefaultModel")
						),
						h("span", { style: cardStyles.hint }, t("lockDefaultModelHint"))
					),
		...
		// ── dedicated settings page (settings.section, v0.3.6) ──────────────

		const sectionStyles = {
			root: { display: "flex", flexDirection: "column", gap: "4px", padding: "4px 0" },
			title: { margin: 0, fontSize: "15px", fontWeight: 600 },
			desc: { margin: 0, opacity: 0.65, fontSize: "12px" }
		};

		/** The full settings page: independent of the plugin-config namespace dispatch. */
		function SubagentModelSection(props) {
			const t = typeof props?.t === "function" ? props.t : (key) => zh[key] ?? en[key] ?? key;
			return h("div", { style: sectionStyles.root },
				h("h3", { style: sectionStyles.title }, t("title")),
				h("p", { style: sectionStyles.desc }, t("description")),
				h(SubagentModelCard, {})
			);
		}
		...
		/**
		 * Client context captured from apply(). The sessions service is read
		 * LAZILY at click time: it is optional, and a controller that activates
		 * after this bundle applied would otherwise leave a permanently null
		 * reference and a silently dead "open child session" button.
		 */
		let clientCtx = null;
		...
		function openChildSession(childId) {
			...
			try {
				const sessions = clientCtx !== null && typeof clientCtx.get === "function" ? clientCtx.get("sessions") : undefined;
		...
		function RunRow(t, run) {
			...
			return h("div", { key: run.runId, style: runStyles.row },
				...
				run.modelSource ? h("span", { style: runStyles.hint }, t(run.modelSource === "arg" ? "srcArg" : run.modelSource === "default" ? "srcDefault" : "srcInherited")) : null,
		...
		function SubagentToolView(props) {
			// Keyed slots do not inject a locale-bound `t`; fall back to the
			// bundled dictionaries so both languages still render.
			const t = typeof props?.t === "function" ? props.t : (key) => zh[key] ?? en[key] ?? key;
			...
			return h("div", { style: runStyles.root },
				...
				h("div", { style: runStyles.meta },
					...
					record !== null && record.modelSource ? h("span", { style: runStyles.hint }, t(record.modelSource === "arg" ? "srcArg" : record.modelSource === "default" ? "srcDefault" : "srcInherited")) : null,
		...
		function apply(ctx) {
			...
			// `ctx.sessions` was read without declaring the inject.) The service
			// itself is resolved per click, not captured here, so a controller
			// that activates later is still picked up.
			clientCtx = ctx;
			ctx.effect(() => ctx.locale.register(NS, { zh, en }), "subagent-model: dictionaries");
			// settings.plugin.item is a KEYED slot since dsh 0.1.1-rc.2 (it was a
			// list slot keyed by `id` before); registration by `key` is the only
			// form the current shell accepts — the v0.3.4 compatibility fix.
			try {
				...
				// A future shell without the plugin-card slot must never break the toolviews.
				try {
					console.warn(`tool-subagent-model: settings plugin card slot unavailable: ${String(error)}`);
				} catch {
					// console itself must not throw either
				}
			}
			// Dedicated settings PAGE (v0.3.6): `settings.section` is a list slot
			// that renders every registration, so this page appears regardless of
			// the namespace-dispatch mechanism — a second, independent surface.
			try {
				const tNav = ctx.locale.bind(NS);
				...
				try {
					console.warn(`tool-subagent-model: settings section slot unavailable: ${String(error)}`);
				} catch {
					// console itself must not throw either
				}
			}
